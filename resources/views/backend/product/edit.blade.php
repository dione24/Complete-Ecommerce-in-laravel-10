...
<div class="card">
    ...
    <div class="card-body">
        <form method="post" action="{{route('product.update',$product->id)}}">
            ...
            <div class="form-group">
                <label for="is_featured">Est mis en avant</label><br>
                <input type="checkbox" name='is_featured' id='is_featured' value='{{$product->is_featured}}'
                    {{(($product->is_featured) ? 'checked' : '')}}> Oui
            </div>
            {{-- {{$categories}} --}}
            ...
            <div class="form-group">
                <label for="price" class="col-form-label">Prix (CFA) <span class="text-danger">*</span></label>
                <input id="price" type="number" name="price" placeholder="Entrer le prix" value="{{$product->price}}"
                    class="form-control">
                @error('price')
                <span class="text-danger">{{$message}}</span>
                @enderror
            ...
            <div class="form-group">
                ...
                <select name="size[]" class="form-control selectpicker" multiple data-live-search="true">
                    ...
                    // dd($data);
                    @endphp
                    <option value="S" @if( in_array( "S" ,$data ) ) selected @endif>Petit</option>
                    <option value="M" @if( in_array( "M" ,$data ) ) selected @endif>Moyen</option>
                    <option value="L" @if( in_array( "L" ,$data ) ) selected @endif> Grand</option>
                    <option value="XL" @if( in_array( "XL" ,$data ) ) selected @endif>Extra Grand</option>
                    @endforeach
                </select>
            </div>
            <div class="form-group">
            ...
            <div class="form-group">
                ...
                <select name="condition" class="form-control">
                    ...
                    <option value="hot" {{(($product->condition=='hot')? 'selected':'')}}>Chaud</option>
                </select>
            </div>
...
<script>
    ...
    $('#summary').summernote({
      placeholder: "Écrire une description courte.....",
        tabsize: 2,
        height: 150
    });
    });
    $(document).ready(function() {
      $('#description').summernote({
        placeholder: "Écrire une description détaillée.....",
          tabsize: 2,
          height: 150
      });
    });
            ...
            if(cat_id !=null){
                ...
                $.ajax({
                    ...
                    success:function(response){
                        ...
                        var html_option="<option value=''>--Sélectionner une sous-catégorie--</option>";
                        if(response.status){
